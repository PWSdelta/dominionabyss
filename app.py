...
# Route to generate strategy and save it
@app.route('/card/<set_slug>/<card_slug>/generate', methods=['POST'])
def generate_strategy_route(set_slug, card_slug):
    # Convert slugs
    card_name = slug_to_name(card_slug)
    set_name = slug_to_name(set_slug)

    print(f"[GENERATE] card_name='{card_name}', set_name='{set_name}'")

    # Find the card in Mongo
    card_doc = cards.find_one({
        "card_name": {"$regex": f"^{card_name}$", "$options": "i"},
        "set_name": {"$regex": f"^{set_name}$", "$options": "i"}
    })
    ...
    if not card_doc:
        print("[ERROR] Card not found for generation!")
        return "Card not found", 404

    # Generate strategy text via Ollama
    strategy_text = generate_strategy(card_doc)

    # Save back to MongoDB
    cards.update_one(
        {"_id": card_doc["_id"]},
        {"$set": {"strategy_review": strategy_text}}
    )

    print("[SUCCESS] Strategy review saved.")
...
def batch_generate():
    generated_cards = []
    missing_cards = list(cards.find({"strategy_review": {"$exists": False}}))

    total_count = cards.count_documents({})

    print(f"[BATCH] Found {len(missing_cards)} cards missing strategy_review.")
    print(f"[BATCH] Total cards in collection: {total_count}")

    for idx, card in enumerate(missing_cards, start=1):
        print(f"[BATCH] ({idx} of {total_count}) Generating for {card['card_name']} ({card['set_name']})")
        strategy_text = generate_strategy(card)
        ...
        generated_cards.append({
            ...
            "strategy_review": strategy_text
        })

    # Return a simple HTML page listing results
    html_template = """
    <!DOCTYPE html>
    <html>
    <head>
    ...
    <body class="container my-4">
      <h1>Batch Generation Complete</h1>
      <p class="lead">{{ generated_count }} strategies generated and saved.</p>
      <div class="list-group">
        {% for card in generated_cards %}
          <div class="list-group-item">
            <h5>{{ card.card_name }} ({{ card.set_name }})</h5>
            <div>{{ card.strategy_review }}</div>
          </div>
        {% endfor %}
      </div>
...
def unanalyzed():
    # Only cards missing a strategy_review
    missing = list(cards.find({"card_name": {"$exists": True}, "set_name": {"$exists": True}, "strategy_review": {"$exists": False}}))
    return render_template("unanalyzed.html", cards=missing)
